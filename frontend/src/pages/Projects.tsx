    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <input value={name} onChange={(e)=>setName(e.target.value)} placeholder="Nome do projeto" className="border rounded-md px-2 py-2 bg-transparent" />
        ...
        {items.map(p => (
          <div key={p.id} className="border rounded-xl p-4 space-y-3">
            ...
            <div className="flex items-center gap-6 text-sm">
              ...
              <span className="opacity-80">{stats[p.id]?.subprojects ?? 0} Subprojetos</span>
            </div>
          </div>
        ))}
        {!items.length && (
          <div className="border rounded-xl p-6 text-center text-sm opacity-70">
            Nenhum projeto criado ainda — use “+ Novo Projeto”.
          </div>
        )}
        <div className="border border-dashed rounded-xl p-4 grid place-items-center text-sm opacity-70">
